...
Entity::Entity(Model &model, float position, float scale, float angle_degrees)
    : _mesh(model.get_mesh()), _material(model.get_material()), _position(position), _word_position(0.0f),
      _scale(scale), _angle_degrees(angle_degrees), _rotation_dir(0.0f, 1.0f, 0.0f){};
...
void Entity::set_rotation_dir(float x, float y, float z) {
    _rotation_dir.x = x;
    _rotation_dir.y = y;
    _rotation_dir.z = z;
}
