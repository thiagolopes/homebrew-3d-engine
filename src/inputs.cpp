...
void Keyboard::set_state(const int key, const bool action) {
    if (key_in_bounds(key))
    {
...
bool Mouse::get_state(MouseKey key) { return state[key]; }
...
void Mouse::update_position(float x, float y) {
...
MousePosition Mouse::get_last_position() { return last_position; }

MousePosition Mouse::get_diff_moviment() {
    return MousePosition{pointer_position.x - last_position.x, last_position.y - pointer_position.y};
}

void Mouse::set_state(const int key, const bool action) { state[(MouseKey)key] = action; }
